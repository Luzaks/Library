...
function showForm(formCreate, id = 1) {
  formCreate
    ...
    : (document.getElementById("change-status-form").style.display = "block");
  document
    .getElementById("updateBTN")
    .setAttribute("onclick", `updateStatus(${id});hideForm()`);
}

function hideForm(formCreate) {
  formCreate
    ? (document.getElementById("addBook").style.display = "none")
    : (document.getElementById("change-status-form").style.display = "none");
  cleanForm();
}
    ...
    booksInfo += `<tbody>
                        <tr>
                            ...
                            <td><a href="#" class="read-status-link" onclick="showForm(false, ${
                              ...
                              item.readed ? "Readed" : item.notReaded ? "Not Readed" : item.reading ? "Reading" : "Set a Status"
                            }</a></td>
                            <td>
                                <img class="remove-book-info" onclick="removeBook(${
                                  item.book_id
                                })" id="remove-book-btn" style="width: 30px; height: 30px; cursor: pointer" src="https://img.icons8.com/plasticine/100/000000/filled-trash.png" alt="remove-button"/>
                            </td>
                        </tr>
                       </tbody>`;
  });
